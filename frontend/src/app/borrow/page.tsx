...
const BorrowPage = () => {
    ...
    useEffect(() => {
        ...
        if (!account && !balanceLoading && Number(balance) > Number('0.5')) {
            toast({
                variant: 'destructive',
                title: 'Wallet balance greater than 0.5 ETH',
                description: 'Wallet balance mut be lower than 0.5 ETH to borrow',
            });
        }
    }, [isConnected, toast]);
            ...
            <Card className="w-full max-w-md backdrop-blur-lg shadow-lg border rounded-lg p-4">
                ...
                <CardContent className="space-y-4">
                    {account && (
                        <>
                            <div className="mb-4">
                                <label htmlFor="walletAddress" className="block text-sm font-semibold mb-2 text-gray-800 dark:text-gray-200">
                                    Wallet Address
                                </label>
                                <Input
                                    id="walletAddress"
                            ...
                            <Button
                                disabled={!isConnected || !account || isSubmitting || balanceLoading || (Number(balance) > Number('0.5'))}
